...
Also includes stats from Apple, Spotify, YouTube, and other platforms.
...
https://s3.us-west-2.amazonaws.com/prod.assets.website.virginmusic.com/wp-content/uploads/2026/02/12184057/virgin.svg


## Data

The pages are fully static and load their content client-side from two JSON
files under [`public/data/`](public/data/):

- `artists.json` — the artist roster shown on `/artists` and the home rotator.
- `news.json` — the aggregated music news feed shown on `/news`.

`news.json` is generated from the RSS sources in
[`src/lib/feeds.ts`](src/lib/feeds.ts) (Billboard, Rolling Stone, Variety, NYT
Popcast, Hypebot, Music Business Worldwide, Digital Music News), limited to the
most recent few per source and interleaved so publications alternate. Refresh
it with:
